...
export default function MovieDetails() {
  ...
  useEffect(() => {
    axios.get(`http://localhost:5000/api/get-movie/${id}`)
  ...
  const handleBookTicket = () => {
    const hasEnglish = movie.languages?.includes("English");
    const hasOnlyHindi = movie.languages?.length === 1 && movie.languages[0] === "Hindi";

    if (movie?.rating === "A") {
      setShowWarning(true); // Show warning first
    } else if (hasEnglish) {
      setShowFormatModal(true); // Show format modal directly if English is available
    } else if (hasOnlyHindi) {
      const selectedLanguage = movie.languages[0]; // This will be "Hindi"

      // ✅ Save to localStorage and navigate without any modal
      ...
      localStorage.setItem("movieLanguage", selectedLanguage); // ✅ fixed
      ...
      navigate("/booking-details", {
        state: {
          ...
          language: selectedLanguage, // ✅ fixed
          image: movie.image
        },
      });
    }
  ...
  // console.log("Sending Image:", movie.image); // This should not be undefined
  ...
  const handleFormatSelect = (screen, language) => {
    ...
    localStorage.setItem("movieGenre", movie.genre);
    localStorage.setItem("movieReleaseDate", movie.releaseDate);
    localStorage.setItem("movieLanguage", language); // ✅ Save selected language
    localStorage.setItem("movieScreen", screen); // Optional, if you want format too
    ...
    <>
      ...
      <div className="mobile-container">
        ...
        <div className="movie-card">
          ...
          <div className="tag-containerz">
            <div className="tag-row">
              <span className="tag-box">
                {Array.isArray(movie.screen) ? movie.screen.join(', ') : ''}
              </span>
            </div>
            ...
            <div className="tag-row">
              <span
                className="tag-box"
                onClick={() => setShowAllLanguages(!showAllLanguages)} // ✅ toggle on click
                style={{ cursor: movie.languages?.length > 2 ? "pointer" : "default" }} // only pointer if more than 2
              >
          ...
          <div className="onprime">
            <img src="https://assets-in-gm.bmscdn.com/promotions/cms/creatives/1748336862612_1stjuneamazonprimeawarenessmsp.jpg" alt="" />
          </div>
        ...
        >
          <div className="overlay">
            ...
            <div className="right-panel">
              ...
              <div className="tag-row">
                <span className="tag-box">
                  {Array.isArray(movie.languages) ? movie.languages.join(', ') : ''}
                </span>
              </div>
